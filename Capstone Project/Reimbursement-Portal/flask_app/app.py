...
def unauthorized(e):
    app.logger.error('Unauthorized Access to Page: %s',request.path)
    return 'Unauthorized Access to the Page.Please login for authorization.',400
...
def method_not_allowed(e):
    ...
    return 'The request method is not supported for the requested resource',405

#Exception Handling
@app.errorhandler(409)
def conflict(e):
    app.logger.error('Conflict: %s',request.path)
    return 'The request could not be completed due to a conflict with the current state of the resorce',409
